...
const CONTACT = gql`
    mutation centerRequest($name:String,$email:String,$phone:String,$message:String){
        centerRequest(input: {
            name: $name,
            email: $email,
            phone: $phone,
...
const JoinUsAsCenter = () => {
    ...
    return (
        <Box className={classes.container}>
            <Button className={classes.btn} variant={"contained"} onClick={() => setOpen(p => !p)}>Join us as a
                center</Button>
                    ...
                    <form method={"post"} onSubmit={submitForm}>
                        <Container className={classes.container}>
                            <Box className={classes.formContainer}>
                                <Typography variant={"h1"} className={classes.formTitle}>Collaboration form</Typography>
                                <Typography variant={"body1"} className={classes.subTitle}>Fill this form to collaborate with us.</Typography>
                                {(e || error) &&
                                <Collapse in={e || error}>
                                ...
                                <TextField className={classes.input} variant={"outlined"} label={"E-mail"}
                                           type={"email"}
                                           value={email}
                                           size={"small"}
                                           onChange={e => {
                                ...
                                <TextField className={classes.input} variant={"outlined"} label={"Phone"}
                                           type={"number"}
                                           value={phone}
                                           size={"small"}
                                           onKeyDown={e => {
                                ...
                                <Button className={classes.submitBtn} onClick={submitForm} disabled={loading}
                                        type={"submit"}
                                        size={"small"}>
                                    {loading ? <CircularProgress size={25} style={{color: "white"}}/> : "Submit"}
                                </Button>
                            </Box>
                        </Container>
