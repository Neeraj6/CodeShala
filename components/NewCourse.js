...
const EDITCOURSE = gql`
    mutation EDITCOURSE($title:String, $description:String, $cover:String, $price:Float, $discountPrice:Float, $slug:String) {
...
const AddCourse = ({open, setOpen,refetch}) => {
    const classes = useStyles();
    const [createCourse, {data, loading, error}] = useMutation(EDITCOURSE);
    ...
    const submitForm = () => {
        if (title?.length > 0 && parseFloat(price) > 0 && parseFloat(discountPrice) < parseFloat(price) && slug?.length > 0) {
            createCourse({
                variables: {
                    title, description,
            ...
            <Box className={classes.wrapper}>
                ...
                <Grid container spacing={3} alignItems={"center"} justify={"center"}>
                    <Grid item sm={12} md={7}>
                        <Box display={"flex"} flexDirection={"column"}>
                            ...
                            <FormControl style={{marginBottom: 15}}>
                                <TextField label={"Slug"} size={"small"} variant={"outlined"}
                                           value={slug}
                    ...
                    <Grid item sm={12} md={5}>
                        <Box display={"flex"} flexDirection={"column"}>
                            <img src={cover ? `${process.env.S3_URL}${cover}` : "/assets/about-us-laptop.svg"} alt={""}
        ...
        <>
            <Button variant={"contained"} component={"label"} size={"small"}
                ...
                <Typography variant={"body1"}
                            className={classes.fileInputText}>Upload &nbsp;&nbsp;&nbsp;
                    <BackupOutlinedIcon/>
                </Typography>
